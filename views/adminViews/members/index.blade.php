...
@section('content')
    <div class="d-flex justify-content-between align-items-center mt-5">
                ...
                <ul class="dropdown-menu">
                    <li>
                        <a class="dropdown-item {{ $sort == 'id' && $order == 'ASC' ? 'active' : '' }}"
                            href="{{ route("members?keyword=$keyword&sort=id&order=ASC&page=$page") }}">
                            ID Tăng dần
                        </a>
                    </li>
                    <li>
                        <a class="dropdown-item {{ $sort == 'id' && $order == 'DESC' ? 'active' : '' }}"
                            href="{{ route("members?keyword=$keyword&sort=id&order=DESC&page=$page") }}">
                            ID giảm dần
                        </a>
                    </li>
                    <li>
                        <a class="dropdown-item {{ $sort == 'name' && $order == 'ASC' ? 'active' : '' }}"
                            href="{{ route("members?keyword=$keyword&sort=name&order=ASC&page=$page") }}">Tên tăng
                            dần</a>
                    </li>
                    <li>
                        <a class="dropdown-item {{ $sort == 'name' && $order == 'DESC' ? 'active' : '' }}"
                            href="{{ route("members?keyword=$keyword&sort=name&order=DESC&page=$page") }}">Tên giảm
                            dần</a>
                    </li>
                </ul>
            </div>
        ...
        <form class="d-flex" action="{{ route("members?order=$order&page=$page") }}" method="GET">
    ...
    @if (isset($_SESSION['success']))
        ...
        @endphp
    @endif
    <form id="formDelete" action="{{ route('members/delete') }}" method="post">
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">
            ...
            <tbody>
                @foreach ($members as $member)
                    <tr>
                        <td>
                            <input class="checkbox" type="checkbox" name="ids[]" value="{{ $member['id'] }}">
                        </td>
                        <td>{{ $member['id'] }}</td>
                        <td>{{ $member['name'] }}</td>
                        <td>
    ...
    <nav class="d-flex justify-content-center align-items-center mt-5">
        <ul class="pagination">
            <li class="page-item">
                <a class="page-link"
                    href="{{ route("members?keyword=$keyword&sort=$sort&order=$order&page=" . max(1, $page - 1)) }}"
                    aria-label="Previous">
            ...
            @for ($i = 1; $i <= $totalPage; $i++)
                <li class="page-item {{ $page == $i ? 'active' : '' }}"><a class="page-link"
                        href="{{ route("members?keyword=$keyword&sort=$sort&order=$order&page=$i") }}">{{ $i }}</a>
                </li>
            @endfor
            <li class="page-item">
                <a class="page-link"
                    href="{{ route("members?keyword=$keyword&sort=$sort&order=$order&page=" . min($totalPage, $page + 1)) }}"
                    aria-label="Next">
